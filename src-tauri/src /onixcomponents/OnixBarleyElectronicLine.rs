use printpdf::*; // 导入 printpdf 库，用于创建 PDF 文档
use printpdf::Pt;
use printpdf::Line;
...
pub fn draw(current_layer: &PdfLayerReference, json_str: &str, page_height: Mm) {
  ...
  let top = convert_to_pdf_coordinates(page_height, Mm(original_top as f32), Some(Mm(px2pt(border_size) as f32)));
  ...
  // 线条终点
  let end_point = Point { 
      x: Pt::from(Mm((left + width) as f32)), 
      y: Pt::from(top) 
  };
