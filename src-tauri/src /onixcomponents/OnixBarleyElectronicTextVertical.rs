// src/onixcomponents/OnixBarleyElectronicTextVertical.rs
use printpdf::*;
use std::fs;
...
pub fn draw(current_layer: &PdfLayerReference, json_str: &str, doc: Option<&PdfDocumentReference>, page_height: Mm, subset_font_path: String) {
    ...
    println!("进入 OnixBarleyElectronicText");
    ...
    // 如果有 `doc` 参数，则加载字体并绘制文本
    if let Some(doc) = doc {
        let font_path = get_font_path(&font_name, &font_style);
        println!("【桌面端-OnixBarleyElectronicTextVertical- subset_font_path: {}", subset_font_path);
        let (font, subset_font_path) = get_or_add_font(doc, subset_font_path, font_path);

        let font_data = fs::read(&subset_font_path).expect("无法读取字体文件");
        let text_x = left;
        ...
        draw_text_with_wrapping(current_layer, &content,  font_size, height, text_x, text_y, &font, &font_data, line_height, letter_spacing, justify_content, align, rotate);
    } else {
        println!("文档对象不存在，无法加载字体和绘制文本");
    }
    
    println!("纵向文本【{}】绘制完成", content);
}
...
fn draw_text_with_wrapping(
    ...
    font_data: &[u8],
    line_height: Mm,
    letter_spacing: f64, // px单位
    justify_content: &str,  // 横向对齐方式
    align: &str, // 纵向对齐方式
    rotate: f64, // 旋转角度
) {
    ...
    let total_text_height = text_line_height * content.lines().count() as f32;
    text_y = adjust_y_for_top_alignment(text_y, font_size, font_data) - (font_size * 2.0);

    match align {
        "flex-end" => {
            text_y = text_y - total_text_height;
        }
        "center" => {
            text_y = text_y - total_text_height / 2.0;
        }
        _ => {} 
    }
    ...
    let rotate: f64 = 90.0 + rotate;

    // 如果旋转角度超过180度，使用sin计算调整位置
    let (translate_x, translate_y, adjustment) = if rotate > 180.0 {
        let angle_rad = rotate - 180.0 - 90.0;
        let adjustment = total_text_height.0 as f64 * angle_rad.to_radians().cos();
        (text_x_pt, text_y_pt - mm2pt(max_width.0 as f64 - adjustment) as f32, adjustment)
    } else {
        (text_x_pt, text_y_pt, 0.0)
    };

    let translate_y_offset = if rotate >= 90.0 && rotate < 270.0 {
        font_size_pt as f32
    } else {
        0.0
    };

    current_layer.set_ctm(CurTransMat::Translate(Pt(translate_x), Pt(translate_y + translate_y_offset)));
    current_layer.set_ctm(CurTransMat::Rotate(rotate as f32 * -1.0));
    current_layer.set_ctm(CurTransMat::Translate(Pt(-text_x_pt), Pt(-text_y_pt - mm2pt(adjustment) as f32 + translate_y_offset)));
...
// 获取字符宽度
fn get_char_width(ch: char, font_size: f64, font_data: &[u8]) -> f64 {
...
// 计算一行文本的宽度
fn calculate_line_width(line: &str, font_size_pt: f64, font_data: &[u8], letter_spacing_mm: f64, max_width: Mm) -> f64 {
